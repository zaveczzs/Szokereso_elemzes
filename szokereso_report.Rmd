...
The source of the data is a previous study of our lab. The raw output file gained from the task was processed by a Python script to yield the recent file format (which is tidy already). However, in the original file, every measure is calculated for each matrices which were included during the same session, so as first step of the analysis, I made averages for each session of these 5 matrices.
```{r}
...
```
Based on the boxplots, there are no very extreme outliers (most extreme datas are in the case of the RTs of the first 3 words), however participant 310 seems to be outlier on several measures, but those are not those that influence the effective performance of participant on the task (not outlier in score of the task, number of found words, but outlier in the number of overall attempts, and repetition).
...
szokereso_session %>% 
  ...
  shapiro.test()
```
Even after data transformation, distributation of overall attempts (Összesütés) still violates normality, so I did not include this measure to the ANOVAs (it has no attribution to better performance, probably reflects more a personal strategy in solving the task).
...
repetition_model
```
Condition of sphericity has been met, but the model was not signficant (F(2, 38)= 0.60, p = 0.55).
